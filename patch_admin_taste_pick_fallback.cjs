...
const targetStr1 = `    const tastePickSubs = (storeConfig?.tastePickSubcategories || []).map((sc) => ({`;
const replacementStr1 = `    const currentTastePicks = storeConfig?.tastePickSubcategories?.length ? storeConfig.tastePickSubcategories : ${fallbackData};
    const tastePickSubs = currentTastePicks.map((sc) => ({`;

const targetStr2 = `                              {(storeConfig?.tastePickSubcategories || []).map((flavor) => {`;
const replacementStr2 = `                              {(storeConfig?.tastePickSubcategories?.length ? storeConfig.tastePickSubcategories : ${fallbackData}).map((flavor) => {`;
