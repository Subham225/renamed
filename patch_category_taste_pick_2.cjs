...
const targetStr = `      {categoryId === 'cakes' && isMainCategory && (
...
const replacementStr = `      {categoryId === 'cakes' && isMainCategory && storeConfig?.tastePickSubcategories && storeConfig.tastePickSubcategories.length > 0 && (
        <div className="max-w-7xl mx-auto px-4 py-10 w-full border-t border-slate-100">
          <div className="mb-6 flex justify-between items-end">
            <div>
              <h3 className="text-2xl font-black text-slate-900 tracking-tight">Your Taste Pick</h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {storeConfig.tastePickSubcategories.map((sub, i) => (
              <div 
                key={sub.id || i}
                onClick={() => onSelectCategory?.(sub.id as any)}
...
if (code.includes(targetStr)) {
  ...
  console.log('Patched CategoryDetailPage.tsx successfully.');
} else {
